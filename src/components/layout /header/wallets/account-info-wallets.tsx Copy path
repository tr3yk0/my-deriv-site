        ...
        <div className='acc-info__wallets-account-type-and-balance'>
            ...
            >
                {!currency ? (
                    ...
                    `${formatMoney(currency, displayBalance ?? 0, true)} ${display_code}`
                )}
            </Text>
        </div>
    ) : null;
...
const AccountInfoWallets = observer(({ is_dialog_on, toggleDialog }: TAccountInfoWallets) => {
    ...
    // Get balance considering swap state
    const originalBalanceData = all_accounts_balance?.accounts?.[loginid ?? ''];
    ...
    // Pass true for isActiveAccount since this is for the current active account
    const accountDisplay =
        loginid && accountDataWithBalance
            ? getAccountDisplayInfo(loginid, accountDataWithBalance, all_accounts_balance, true)
            : null;
